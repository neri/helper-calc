# ライブコーディング用設計書: アプリスイッチャー

## 概要
このプロジェクトは、複数のサブアプリを切り替えられるアプリスイッチャー (Single Page Application) を実装します。メインアプリがサブアプリを管理し、ユーザーが簡単に切り替えられるようにします。アプリはオフラインで動作し、外部通信を行いません。ライブコーディングのデモンストレーションとして、シンプルで理解しやすい設計を採用します。
...
│   ├── sub-app.ts      # サブアプリインターフェース
│   ├── helper-calc-app.ts # サブアプリ: 助っ人貸出回数計算機
│   ├── dummy-app.ts    # サブアプリ: ダミーアプリ
│   ├── calculator.ts   # 計算ロジック (共有)
│   └── style.css       # CSSファイル
├── package.json        # 依存関係
